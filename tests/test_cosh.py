...
def test_cosh_known_values():
    ...
    assert np.isclose(cosh(1), np.cosh(1)), "should match numpy's cosh(1)"
    assert np.isclose(cosh(-1), np.cosh(-1)), "should match numpy's cosh(-1)"


def test_cosh_against_numpy():
    # Test against numpy's cosh function for a range of small values
    x_values = np.array([-5, -2, -1, 0, 1, 2, 5])
    for x in x_values:
        assert np.isclose(
            cosh(x), np.cosh(x), rtol=1e-5
        ), f"cosh({x}) should match numpy's cosh({x})"
...
def test_cosh_array_input():
    ...
    ), "cosh(array) should match numpy's cosh(array)"
...
def test_cosh_small_values():
    ...
    ), "cosh(small values) should match numpy's cosh(small values)"
...
def test_cosh_negative_values():
    ...
    assert np.allclose(
        result, expected, rtol=1e-5
    ), "cosh(negative values) should match numpy's cosh(negative values)"
