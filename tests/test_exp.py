...
def test_exp_known_values():
    ...
    assert np.isclose(
        exp(1), np.e
    ), "exp(1) should be approximately equal to e (~2.71828)"
    assert np.isclose(
        exp(-1), 1 / np.e
    ), "exp(-1) should be approximately 1/e (~0.36788)"
...
def test_exp_against_numpy():
    ...
    for x in x_values:
        assert np.isclose(exp(x), np.exp(x)), f"should match numpy exp({x})"
...
def test_exp_array_input():
    ...
    result = exp(x_array)
    assert np.allclose(result, expected), "should match numpy's exp(array)"
...
def test_exp_large_values():
    ...
    assert np.allclose(
        result, expected
    ), "exp(large values) should match numpy's exp(large values)"
...
def test_exp_small_values():
    ...
    assert np.allclose(
        result, expected
    ), "exp(small values) should match numpy's exp(small values)"
...
def test_exp_negative_values():
    ...
    assert np.allclose(
        result, expected
    ), "exp(negative values) should match numpy's exp(negative values)"
