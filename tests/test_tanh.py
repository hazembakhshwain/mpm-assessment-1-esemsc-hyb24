...
def test_tanh_known_values():
    ...
    assert np.isclose(tanh(1), np.tanh(1)), "should match numpy tanh(1)"
    assert np.isclose(tanh(-1), np.tanh(-1)), "should match numpy tanh(-1)"


def test_tanh_against_numpy():
    # Test against numpy's tanh function for a range of small values
    x_values = np.array([-5, -2, -1, 0, 1, 2, 5])
    for x in x_values:
        assert np.isclose(
            tanh(x), np.tanh(x), rtol=1e-5
        ), f"tanh({x}) should match numpy's tanh({x})"
...
def test_tanh_array_input():
    ...
    ), "tanh(array) should match numpy's tanh(array)"
...
def test_tanh_small_values():
    ...
    ), "tanh(small values) should match numpy's tanh(small values)"
...
def test_tanh_negative_values():
    ...
    assert np.allclose(
        result, expected, rtol=1e-5
    ), "tanh(negative values) should match numpy's tanh(negative values)"
